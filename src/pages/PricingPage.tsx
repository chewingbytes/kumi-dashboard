      ...
      <main className="max-w-6xl mx-auto px-6 md:px-10 py-12 space-y-10">
        ...
        >
          <TabsList className="grid grid-cols-2 w-full max-w-md mx-auto rounded-full border-2 border-[#111827] bg-white shadow-[6px_6px_0px_#111827]">
            <TabsTrigger
              value="email"
              className="data-[state=active]:bg-[#8B5CF6] data-[state=active]:text-white font-bold rounded-full"
            >
              Email
            </TabsTrigger>
            <TabsTrigger
              value="whatsapp"
              className="data-[state=active]:bg-[#10B981] data-[state=active]:text-white font-bold rounded-full"
            >
              WhatsApp
            </TabsTrigger>
          </TabsList>
  ...
  return (
    <div className="space-y-6">
      <div className="border-2 border-[#111827] bg-white rounded-3xl shadow-[10px_10px_0px_#111827] p-6 md:p-8 relative overflow-hidden">
        ...
        />
        <div
          className="absolute -left-16 bottom-0 w-52 h-52 rotate-6 opacity-10"
          style={{
            background: `linear-gradient(135deg, ${plan.accent}, #FDFBF5)`,
          }}
        />
        ...
        <div className="flex items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-[#475569]">
              Drag the slider to see estimated monthly cost.
            </p>
          </div>
          <div
            className="hidden md:flex w-14 h-14 rounded-2xl border-2 border-[#111827]"
            style={{ background: plan.accent }}
          >
        ...
        <div className="grid md:grid-cols-[1.1fr_0.9fr] gap-6 mt-6">
          <div className="border-2 border-[#111827] rounded-2xl p-5 bg-white/80 shadow-[6px_6px_0px_#111827] space-y-3">
            ...
            <p className="text-sm text-[#475569]">
              Includes base ${plan.base.toFixed(0)} + ~{mid.toFixed(0)} students
              @ ${plan.perStudent.toFixed(2)} each.
            </p>
            <div className="grid grid-cols-2 gap-3 text-sm text-[#111827]">
              <Stat label="Base" value={`$${plan.base.toFixed(2)}`} />
              <Stat
                label="Per student"
                value={`$${plan.perStudent.toFixed(2)}`}
              />
              <Stat label="Avg students" value={`${mid.toFixed(0)}`} />
              <Stat
                label="Overage"
                value={`$${plan.overage.toFixed(2)}/student`}
              />
            </div>
          </div>
